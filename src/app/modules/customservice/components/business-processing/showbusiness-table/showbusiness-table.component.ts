...
import { businessProcessingCustomFieldData } from '../../../service/business-processingCustomFieldData';
import { BusinessProcessingService } from '../../../service/business-processing.service';


@Component({
  selector: 'app-showbusiness-table',
  templateUrl: './showbusiness-table.component.html',
  styleUrls: ['./showbusiness-table.component.scss']
})
export class ShowbusinessTableComponent implements OnInit, OnChanges, OnDestroy {
  @Input() filterData;
  ...
  tableTreeData = [];
  expandDataCache = {};
  constructor(private businessProcessingService: BusinessProcessingService, private dropoutComplaintProcessFormService: DropoutComplaintProcessFormService,
  ...
  getFilterField() {
    const newData = this.customColumnDialogService.filterSelectColoumn(businessProcessingCustomFieldData);
  ...
  exportsFile() {
    ...
    this.businessProcessingService.saveDealCycleQueryVO(this.exportsParams).subscribe(res => {
      this.businessProcessingService.exportDealCycle({ ridesKey: res.result });
    });
  }
  ...
  getDivisonNetSaleData() {
    ...
    this.subscribeAll$['queryDealCycle$'] = this.businessProcessingService.queryDealCycle(this.targetNetSaleParams)
      .subscribe(res => {
  ...
  getTableTreeData(data: object = {}) {
    ...
    if (!(data['children'] && data['children'].length)) {
      this.loading = this.dropoutComplaintProcessFormService.loading = true;
      this.subscribeAll$['queryDealCycle$'] = this.businessProcessingService.queryDealCycle(params).subscribe(res => {
  ...
  initTreeTable(data = {}): void {
    const { plusPercentage } = this.businessProcessingService;
